...
export default function AdminDashboard({ onLogout }: Props) {
  ...
  const active = passes.filter(p => p.status === "active" && !isPassExpired(p)).length;
  const expired = passes.filter(p => p.status === "active" && isPassExpired(p)).length;
  ...
  const toggleStatus = async (id: string, newStatus: "active" | "revoked") => {
    ...
    await updatePassStatus(id, newStatus, reason);
    await logActivity(`pass_${newStatus}`, `Pass for ${pass.fullName} was ${newStatus}.`);

    if (newStatus === "revoked") {
      supabase.functions.invoke('send-pass-email', {
        body: { fullName: pass.fullName, email: pass.email, code: pass.code, type: 'revoked', reason, photo: pass.photoUrl || "" },
  ...
  const addDomain = () => {
    ...
    if (!d || approvedDomains.includes(d)) return;
    const updated = [...approvedDomains, d];
  ...
  const removeDomain = (domain: string) => {
    ...
    const updated = approvedDomains.filter(dd => dd !== domain);
    saveApprovedDomains(updated);
    setApprovedDomains(updated);
          ...
          <motion.div variants={itemVariants} className="flex-1 min-w-[250px] space-y-5">
            <Card className="shadow-card">
              <CardContent className="pt-5">
                ...
                >
                  <h3 className="font-display font-bold">Approved Domains ({approvedDomains.length})</h3>
            ...
            <Card className="shadow-card">
              <CardContent className="pt-5">
                ...
                <div className="max-h-72 overflow-y-auto space-y-2">
                  ...
                  {activityLog.length === 0 && <p className="text-muted-foreground text-xs">No activity yet</p>}
                </div>
              </CardContent>
            </Card>
          </motion.div>
